...
# ----------------------------------------------------------------------------------------
def run_optimization(seq_template, backend, d, target_evolution, init_state, trace_index, loss_function,  mode='simple', max_iterations=5, times=None):
    """Run an optimization
    Args:
        seq_template: template for one layer of gates
        backend: backend for simulation
        d: layer depth
        target_evolution: target evolution
        init_state: initial qubit-qumode state
        trace_index: which component to keep from partial trace
        loss_function: cost function to minimize
    ...
    init_guess = (np.random.rand(total_params) - 0.5) * 2.0  # range [-1, 1]

    if mode == 'coordinate-descent':
        return coordinate_descent_optimization(seq_template, backend, d, target_evolution, init_state, trace_index, loss_function, init_guess, max_iterations, times)
    else:
        return simple_optimization(seq_template, backend, d, target_evolution, init_state, trace_index, loss_function, init_guess, times)
# ----------------------------------------------------------------------------------------
# ----------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------
def simple_optimization(seq_template, backend, d, target_evolution, init_state, trace_index, loss_function, init_guess, times=None):
...
# ----------------------------------------------------------------------------------------
def coordinate_descent_optimization(seq_template, backend, d, target_evolution, init_state, trace_index, loss_function, init_guess, max_iter, times=None):
    """
    ...
    for it in range(max_iter):
        for gname, indices in groups.items():
            ...
            def f_block(block_vars):
                ...
                return loss_function(temp, seq_template, backend, d, target_evolution, init_state, trace_index, times)
            ...
            curr_infid = loss_function(curr_params, seq_template, backend, d, target_evolution, init_state, trace_index, times)
